...
from .rotary_embeddings import apply_rope
...
class JointBlock(nn.Module):
    ...
    def __init__(self, dim: int, nhead: int, mlp_ratio: float = 4.0, pre_only: bool = False):
        ...
        self.latent_block = MMDitSingleBlock(dim,
                                             ...
                                             kernel_size=3,
                                             padding=1)
        self.audio_block = MMDitSingleBlock(dim,
                                           nhead,
                                           mlp_ratio,
                                           pre_only=pre_only,
                                           kernel_size=3,
    ...
    def forward(
        ...
        # extended_c: BS * N1 * D
        x_qkv, x_mod = self.latent_block.pre_attention(latent, extended_c, latent_rot)
        c_qkv, c_mod = self.audio_block.pre_attention(audio_cond, global_c, audio_rot)

        latent_len = latent.shape[1]

        joint_qkv = [torch.cat([x_qkv[i], c_qkv[i]], dim=2) for i in range(3)]

        attn_out = attention(*joint_qkv)
        x_attn_out = attn_out[:, :latent_len]
        c_attn_out = attn_out[:, latent_len:]

        latent = self.latent_block.post_attention(latent, x_attn_out, x_mod)
        if not self.pre_only:
            audio_cond = self.audio_block.post_attention(audio_cond, c_attn_out, c_mod)

        return latent, audio_cond
